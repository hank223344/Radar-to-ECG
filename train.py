...
def pearsonr_correlation(x1, x2):
    cos = nn.CosineSimilarity(dim=-1, eps=1e-6)
    pearson = cos(x1 - x1.mean(dim=-1, keepdim=True), x2 - x2.mean(dim=-1, keepdim=True))
    
    return (abs(pearson)).mean()

optimizer = optim.Adam(list(model1.parameters()) + list(model2.parameters()) + list(model3.parameters()), lr=learning_rate)
############################# training ###################################
TT_start = time.time()  ### total training time

def train(num_epochs, model1, model2, model3, train_loader, val_loader):
    # Train the model
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            for validation_step, (val, val_labels) in enumerate(val_loader):
                ...
                pear_loss = pearsonr_correlation(output, label)
                ...
                pearsonr_loss = pearsonr_loss + pear_loss.item()
        ...
        print("\r Train Epoch: %d/%d \tLoss: %.4f \tVal_Loss: %.4f \tVal_Loss: %.4f \ttime: %.2f" % (epoch + 1,
                                                                                    num_epochs,
                                                                                    training_loss / (training_step + 1),
                                                                                    validation_loss / (validation_step + 1),
                                                                                    pearsonr_loss / (validation_step + 1),
                                                                                    end - start))

        training_loss_list.append(training_loss / (training_step + 1))
        validation_loss_list.append( validation_loss / (validation_step + 1))

        np.savez('training_loss', training_loss=training_loss_list, validation_loss=validation_loss_list)
...
training_loss, validation_loss = train(num_epochs, model1, model2, model3, train_loader, val_loader)
