...
class Conv2dSubampling(nn.Module):
    def __init__(self, in_channels: int, out_channels: int) -> None:
        ...
        self.sequential = nn.Sequential(
            ...
            nn.Conv2d(1, 16, kernel_size=(3, 2), stride=(2, 1)),
            nn.BatchNorm2d(16),
            nn.ReLU(),       
            nn.MaxPool2d(kernel_size=(3, 2), stride=(1, 1)),
...
class Conformer_em(nn.Module):
    def __init__(
        ...
        super(Conformer_em, self).__init__()
    ...
    def forward(self, inputs: Tensor, inputs2: Tensor) -> Tuple[Tensor, Tensor]:
        inputs = inputs.unsqueeze(-1)
        inputs = torch.cat((inputs, inputs2), -1)
        #print(inputs.shape)
